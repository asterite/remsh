...
public class CharacterSuggestionProvider extends ContentProvider {
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] args, String sortOrder) {
		...
		for (int i = first; i <= last; i++) {
			...
			String pinyin = Characters.getPinyin(i);
			if (character.equals(query) || keyword.startsWith(query) || pinyin.startsWith(query) || i == queryNum) {
				indices.add(i);
				results.add(character + ": " + keyword + " ("+ pinyin + ")");
			}
		}
	...
	private class SuggestionsCursor extends AbstractCursor {
		...
		public String getString(int column) {
			...
			case 2: return "content://com.asterite.remsh.CharacterSuggestion/" + indices.get(getPosition());
